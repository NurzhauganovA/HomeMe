...
-- -----------------------------------------------------------------------------
-- 9. dashboard_referrallink — админские реферальные ссылки (роль по кампании)
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS dashboard_referrallink (
    id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    ...
    created_at timestamptz NOT NULL DEFAULT NOW(),
    updated_at timestamptz NOT NULL DEFAULT NOW(),
    role_id bigint NOT NULL REFERENCES dashboard_role (id)
        DEFERRABLE INITIALLY DEFERRED
);

CREATE INDEX IF NOT EXISTS dashboard_referrallink_code_idx
    ON dashboard_referrallink (code);

CREATE INDEX IF NOT EXISTS dashboard_referrallink_is_active_idx
    ON dashboard_referrallink (is_active);

CREATE INDEX IF NOT EXISTS dashboard_referrallink_role_id_idx
    ON dashboard_referrallink (role_id);
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'telegram_bot_botuser_referral_link_id_fkey'
    ) THEN
        ALTER TABLE telegram_bot_botuser
            ADD CONSTRAINT telegram_bot_botuser_referral_link_id_fkey
            FOREIGN KEY (referral_link_id) REFERENCES dashboard_referrallink (id)
            ON DELETE SET NULL DEFERRABLE INITIALLY DEFERRED;
    END IF;
END $$;

CREATE INDEX IF NOT EXISTS telegram_bot_botuser_referral_link_id_idx
    ON telegram_bot_botuser (referral_link_id);

-- -----------------------------------------------------------------------------
-- 10. Проверка: колонки botuser (должны быть referral_code, invited_by_id)
-- -----------------------------------------------------------------------------

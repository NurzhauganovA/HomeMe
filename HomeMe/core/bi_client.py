...
class EnhancedBIGroupClient:
    ...
    def get_all_real_estates_office(self) -> List[Dict]:
        """Получает ВСЕ коммерческие объекты (обходим пагинацию)"""
        all_items = []
        page = 1
        page_size = 100
        ...
        while True:
            payload = {
                "companyIds": self.COMPANY_IDS,
                "propertyTypes": self.OFFICE_PROPERTY_TYPES,
                "pageNo": page,
                "pageSize": page_size
            }
            try:
                ...
                all_items.extend(items)
                logger.info(f"📥 Fetched page {page} of Office RealEstates (got {len(items)})")
                ...
                logger.error(f"❌ Error fetching office real estates page {page}: {e}")
                break
    ...
    def get_placements_for_complex_office(self, complex_uuid: str) -> List[Dict]:
        """Получает ВСЕ коммерческие помещения конкретного объекта"""
        all_placements = []
        page = 1
        page_size = 50
